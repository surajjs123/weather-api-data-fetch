...
#     # Calculate the cutoff time - this should go BACK in time, not forward
#     cutoff_time = (datetime.now() - timedelta(hours=hours)).isoformat()
    
#     # Fixed query - removed LIMIT constraint and fixed the logic
#     query = '''
...
# # Fixed API endpoint with better error handling
...
#         # Get data from database - try getting all data first
#         df = get_weather_data_from_db(48)  # Changed to get all data
...
# def export_pdf():
#     """Generate PDF report with weather data chart"""
#     try:
#         # Get data from database
#         df = get_weather_data_from_db(48)
...
#         ax1.legend()
#         ax1.set_title('Temperature and Humidity Over Time (Last 48 Hours)', fontsize=14, fontweight='bold')
...
#             plt.savefig(tmp_chart.name, format='png', dpi=150, bbox_inches='tight')
...
#         subtitle = Paragraph("Temperature and Humidity Analysis", styles['Heading2'])
#         story.extend([title, subtitle, Spacer(1, 20)])
        
#         # Create metadata table
#         metadata = [
#             ['Location:', f'Latitude {lat}°, Longitude {lon}°'],
#             ['Date Range:', f"{df['timestamp'].min().strftime('%Y-%m-%d %H:%M')} to {df['timestamp'].max().strftime('%Y-%m-%d %H:%M')}"],
...
#         # Add chart
#         chart_img = Image(chart_path, width=7*inch, height=5.6*inch)
...
#         <para align="center">
#         <font size="10">
#         Data source: Open-Meteo MeteoSwiss API<br/>
#         Generated by Weather Data Backend Service
#         </font>
#         </para>
#         """
